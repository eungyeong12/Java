...
public class App_BalancedBinarySearchTree {
    public static void main(String[] args) throws IOException {
        final String fout_name = "BST_test.txt";
        Scanner cin = new Scanner(System.in); FileWriter fout = new FileWriter(fout_name);
        int[] intArray = {0, 1, 2, 3, 4, 5}; String BST_name = "BST_IntegerEntry";
        BalancedBinarySearchTree<Integer> bstIntEntry = new BalancedBinarySearchTree<>(BST_name);
        for (int int_entry : intArray) { bstIntEntry.insert_withRebalancing(int_entry); System.out.println(bstIntEntry);
        }bstIntEntry.fprintBST_withDepth(fout);
        BinarySearchTreeNode<Integer> bstn_int;
        int int_searchKey = 1, searchResult;
        while(true) { System.out.printf("Input an int_searchKey (-1 to quit) : ");
            int_searchKey = cin.nextInt();
            if (int_searchKey == -1) break;
            bstn_int = bstIntEntry.search(int_searchKey);
            if ((bstn_int != null) && (int_searchKey == bstn_int.getEntry())){ System.out.printf("int_searchKey(%d) is found in the %s\n", int_searchKey, BST_name);
            } else { System.out.printf("int_searchKey(%d) is not found in the %s\n", int_searchKey, BST_name);
            }
        }
        for (int i=0; i<intArray.length; i++) { BinarySearchTreeNode<Integer> bstn, BST_root = bstIntEntry.getRoot(); bstn = bstIntEntry._deleteBSTN(BST_root); bstIntEntry.setRoot(bstn);
        bstIntEntry.fprintBST_withDepth(fout); }
        String[] strArray =
                { "A", "B", "C", "D", "E", "F", "G"};
